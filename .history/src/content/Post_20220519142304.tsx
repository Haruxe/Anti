...
//   console.log(contentId)
  const { data } = useMoralisQuery(setBlockchainPostArr, (query) => query.equalTo("contentId", contentId));
  console.log(data)
    ...
    useEffect(() => {
        async function getBlockchainPosts() {
            try {
                ...
                // const blockchainContentId = query.equalTo("contentId", contentId)
                if (profile) {
                ...
                // setContentId(blockchainContentId)
            } catch (error) {
                console.log(error)
            }
        }
